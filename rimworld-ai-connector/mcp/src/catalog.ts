...
const state: ToolSpec[] = [
  ...
  t("state.letters", "Letters", "Letters waiting on screen, with id and choices where applicable. Answer with rimworld_ui_letter.",
    { include_archived: z.boolean().optional() }, RO),
...
const map: ToolSpec[] = [
  ...
  t("map.detail", "Building camera (ASCII)",
    ...
    { x: int.optional(), z: int.optional(), around: location.optional().describe("centre on a thing id, pawn, anchor or Room:N"), w: int.min(4).max(60).optional().describe("default 24"), h: int.min(4).max(60).optional().describe("default 24"), roof: z.boolean().optional() }, RO),
  ...
  t("map.terrain_stats", "Terrain stats", "Counts of fertile soil, water, rock, ore (by type) etc. within radius of home.", {}, RO),
...
const ui: ToolSpec[] = [
  ...
  t("ui.area", "Edit areas", "Edit the home area or allowed areas.",
    { action: z.enum(["home_add", "home_remove", "create", "delete", "add", "remove"]), label: z.string().optional(), cells: cells.optional(), rect: locationRect.optional() }, WRITE),
  ...
  t("ui.designate", "Designate work",
    ...
    { designator: z.string(), cells: cells.optional(), rect: locationRect.optional(), things: ids.optional() }, DESTRUCTIVE),
  ...
  t("ui.prisoner", "Prisoner interaction", "Set prisoner interaction mode and medical care.",
    { pawn, mode: z.string().optional().describe("NoInteraction|MaintainOnly|ReduceResistance|AttemptRecruit|Release|Execution|Enslave|Convert|..."), medical: z.string().optional() }, WRITE),
